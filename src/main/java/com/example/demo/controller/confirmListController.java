...
public class confirmListController {
    ...
    public String confirmList(Model model, HttpSession session,
        ...
        if((int)session.getAttribute("role_id")==1){
            model.addAttribute("list", confirmListService.Confirm_in_List(pageNum));
            ...
            model.addAttribute("list", confirmListService.Confirm_in_List(pageNum)); //수정 필요
    ...
    public String confirmList_out(Model model, HttpSession session,
                                  @RequestParam(name = "pageNum", defaultValue = "1") int pageNum){
        if((int)session.getAttribute("role_id")==1){
            model.addAttribute("list", confirmListService.Confirm_out_List(pageNum));
            model.addAttribute("navFragment", "fragment/nav/adminNav");
            ...
            model.addAttribute("list", confirmListService.Confirm_out_List(pageNum)); //수정 필요
            model.addAttribute("navFragment", "fragment/nav/fieldNav");
            model.addAttribute("content", "content/confirmList_out");
        }
        return "layout";
